...
+(AmazonServiceRequest *)createRequest:(EC2DescribeSecurityGroupsRequest *)describeSecurityGroupsRequest
{
    ...
    if (describeSecurityGroupsRequest != nil) {
        if (describeSecurityGroupsRequest.dryRunIsSet) {
            [request setParameterValue:(describeSecurityGroupsRequest.dryRun ? @"true":@"false") forKey:[NSString stringWithFormat:@"%@", @"DryRun"]];
        }
    }
