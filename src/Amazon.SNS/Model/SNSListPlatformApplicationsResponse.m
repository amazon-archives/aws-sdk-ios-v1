...
@implementation SNSListPlatformApplicationsResponse

@synthesize platformApplications;
...
{
    if (self = [super init]) {
        platformApplications = [[NSMutableArray alloc] initWithCapacity:1];
        nextToken            = nil;
    }
...
-(SNSPlatformApplication *)platformApplicationsObjectAtIndex:(int)index
{
    return (SNSPlatformApplication *)[platformApplications objectAtIndex:index];
}
...
-(NSString *)description
{
    ...
    [buffer appendString:@"{"];
    [buffer appendString:[[[NSString alloc] initWithFormat:@"PlatformApplications: %@,", platformApplications] autorelease]];
